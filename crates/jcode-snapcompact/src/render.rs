...
fn encode_indexed_png(
	...
	let mut writer = encoder
		.write_header()
		.map_err(|err| anyhow!("Failed to write PNG header: {err}"))?;
	writer
		.write_image_data(&pack_bits(pixels, width, height, bits, &remap))
		.map_err(|err| anyhow!("Failed to write PNG data: {err}"))?;
	writer
		.finish()
		.map_err(|err| anyhow!("Failed to finish PNG stream: {err}"))?;
	Ok(out)
}
...
pub fn supported_chars(font: &str, chars: &str) -> Result<String> {
...
/// cell.
/// Returns the raw PNG bytes. Native-cell bitmap shapes encode indexed; stretch
/// and TrueType shapes encode RGB. Callers that need base64 (image blocks) encode
/// the returned bytes themselves.
pub fn render_png(text: &str, options: &RenderOptions) -> Result<Vec<u8>> {
	let size = options.size;
	if size == 0 || size > MAX_FRAME_SIZE {
		return Err(anyhow!("Invalid frame size {size}: expected 1..={MAX_FRAME_SIZE}"));
	}
	...
	if !matches!(columns, 1 | 2) {
		return Err(anyhow!("Invalid snapcompact columns {columns}: expected 1 or 2"));
	}
	let doc = columns == 2;
	...
	match font {
		RenderFont::Ttf(font) => {
			...
			encode_rgb_png(&pixels, size, height, png::Compression::Best)
		},
		RenderFont::Bitmap(font) => {
			...
			encode_rgb_png(&frame, size, dst_h, png::Compression::Best)
		},
	}
}
...
mod tests {
	...
	fn opts(size: u32) -> RenderOptions {
		RenderOptions { size, ..Default::default() }
	}
	...
	/// Test shim: `render_png` now returns raw PNG bytes directly, so the
	/// former base64 → bytes decode is the identity. Kept so the ported test
	/// bodies below read unchanged.
	fn png_bytes(bytes: Vec<u8>) -> Vec<u8> {
		bytes
	}

	/// Test-only alias matching the reference's owned-args signature.
	fn render_snapcompact_png_sync(text: String, options: RenderOptions) -> Result<Vec<u8>> {
		render_png(&text, &options)
	}

	#[test]
	fn render_native_is_indexed_and_stretch_is_rgb() {
		let native = png_bytes(
			render_snapcompact_png_sync("Hello world. Again.".into(), RenderOptions {
				size: 128,
				font: Some("8x8".into()),
	...
	fn indexed_png_narrows_palette_and_bit_depth() {
		...
		let sent = png_bytes(
			render_snapcompact_png_sync("Hi. Ok.".into(), RenderOptions {
				size: 128,
				font: Some("8x8".into()),
	...
	fn rejects_bad_shapes() {
		...
		assert!(
			render_snapcompact_png_sync("x".into(), RenderOptions {
				size: 64,
				font: Some("9x9".into()),
				..Default::default()
			})
	...
	fn frame_height_hugs_used_rows() {
		...
		let render = |text: &str, opts: RenderOptions| {
			png_bytes(render_snapcompact_png_sync(text.into(), opts).unwrap())
		};
		let opts_8x8 =
			|| RenderOptions { size: 64, font: Some("8x8".into()), ..Default::default() };
		...
		let repeated =
			render("0123456789", RenderOptions { line_repeat: Some(2), ..opts_8x8() });
		assert_eq!(dims(&repeated), (64, 32));
		// Doc layout counts `\n` lines down the first column.
		let doc = render("Hello there.\nSecond line", RenderOptions {
			size: 256,
			font: Some("8x13".into()),
			cell_width: Some(8),
			cell_height: Some(16),
		...
		let stretched = render("0123456789ab", RenderOptions {
			size: 60,
			font: Some("8x8".into()),
			cell_width: Some(6),
			cell_height: Some(6),
